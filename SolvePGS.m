...
%
% Implementation of the Primal Gradient Scheme (PGS) for
...
function [q_best, iter, timeSteps, errorSteps] = SolvePGS(A, b, varargin)
...
beta = 4; % smoothness parameter -- smaller values are more aggressive
...
while (iter < maxiter)
    ...
    if (L_cond < cond_threshold)
        disp('PGS: Laplacian ill-conditioned. Returning best iterate.'); 
    ...
    % Compute new iterate
    xp = max(delta_border, x .* pgs_scaling); 
