...
%
% Implementation of the Accelerated Gradient Scheme (AGS2) for
...
function [q_best, iter, timeSteps, errorSteps] = SolveAGS2(A, b, varargin)
...
beta = 1.1; % smoothness parameter -- smaller values are more aggressive
...
while (iter < maxiter)
    ...
    if (L_cond < cond_threshold)
        disp('AGS2: Laplacian ill-conditioned. Returning best iterate.'); 
    ...
    % Compute new iterate
    y = max(delta_border, x - x.*gradf/beta); 
    z = max(delta_border, x0 - x0.*totgradf/beta); 
    xp = tau * z + (1-tau) * y; 
